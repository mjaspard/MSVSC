...
if __name__ == '__main__':
	...
	# Test the picker by showing ech stream
	...
	# delete EventID as we want to keep the one from station catalog
	del merged_event['EventID']
	# add 1 column per station
	sta_string = ''
	for sta in dict_sta:
		merged_event[sta] = False
		sta_string = "{},{}".format(sta_string, sta)
	sta_string = sta_string.replace(",", "", 1)	# remove 1st comma

	print(merged_event)
	print('sta_string = ', sta_string)
	keep_columns = "OriginTime , StartTime, {}".format(sta_string)
	# keep_columns = "OriginTime , StartTime"
	# Convert the string variable to a list of column names
	selected_columns = [col.strip() for col in keep_columns.split(',')]
	print(keep_columns)


	# Extract event present in least_station
	dict_sta_event = {}
	for name, param in dict_sta.items():
		...
		for sta_cat in station_CAT:
			...
			# add a column for this station with eventid for this station
			# print(merged_event.columns.tolist())
			merged_event[name] = merged_event[['EventID']]
			print("selected_columns = ", selected_columns)
			merged_event = merged_event[selected_columns]
			print(merged_event)
			print(len(merged_event))
			input("wait...")

	input("Catalog common event done...")
...
# 		print(name, param)
		station_dir = glob.glob("PhD_data_class/CAT_2019/DAT/KV/"+name+"/*/*")
		for sacname in station_dir :
			...
			if not p_arr_man:
				continue
			p_arr_man = p_arr_man - 2 # !!! because signal is strating 100 counte further
			# New picker functiom
			p_arr = p_arr_man
