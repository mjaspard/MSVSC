...
def get_median_value(dict_event, namp_len, whether_plot):
	...
	for key, value1 in dict_event.items():
		...
		for para in dict_sta.values():
			if (value1[para] != []):
				if (len(value1[para][0]) == namp_len):
					...
					tempo_waveform[para] = (value1[para][2])
					print("para = ", para)
					print(type(para))
					print(tempo_waveform)
					input("...")
		...
		if len(tempo_FI)%2 == 1:
			...
			# print("tempo_FI = ", tempo_FI)
			# print("np.median(tempo_FI) = ", np.median(tempo_FI))
			# print("1 median_as  = ", median_as)
			if isinstance(median_as, int):
				median_FI.append(tempo_FI[median_as])
				median_amp.append(tempo_amp[median_as])
				waveform[key] = [median_as, [tempo_waveform[str(median_as)]]]
			else:
			...
			print(tempo_waveform)
			print(median_as)
			print(type(median_as))
			# print("np.median(tempo_FI) = ", np.median(tempo_FI))
			# print("2 median_as  = ", median_as)
			if isinstance(median_as, int):
				median_FI.append(tempo_FI[median_as])
				median_amp.append(tempo_amp[median_as])
				waveform[key] = [median_as, [tempo_waveform[str(median_as)]]]
...
def new_catalog(labels, event_info, events_key, median_FI, sort_idx):
	...
	event_info_temp.to_csv('out_jos/text/new_catalog', index=False, header=True)
				 

def plot_rep(clust, amp, sort_idx, waveform):
	labels = np.unique(clust.labels_)
	num_rep = 100
	mx, my = 10, 10
	amp = np.array(amp)
	...
	for label in labels:
		...
		title2 = ' ('+str(len(idx))+' members)'
		fig.suptitle(title1+title2, fontsize=40)
		title = 'out_jos/png/wf'+str(label)+'.pdf'
		plt.savefig(title, format='pdf')
		plt.close()
...
# def plot_stream_cls(clust, amp, sort_idx):
# 	labels = np.unique(clust.labels_)
# 	num_rep = 100
# 	mx, my = 10, 10
# 	amp = np.array(amp)
...
###########################################################################
	...
	input("wait...")
	print("median_FI  = ", median_FI)
	print("median_FI lentgh  = ", len(median_FI))
	input("click to continue...")	
	print("median_amp = ", median_amp)
	print("median_amp lentgh  = ", len(median_amp))
	input("click to continue...")		
	print("events_key  = ", events_key)
	print("events_key lentgh  = ", len(events_key))
	input("click to continue...")	
	print("waveform  = ", waveform)
	print("waveform lentgh  = ", len(waveform))
	input("click to continue...")	
	...
	input("click to continue...")		
	plot_rep(clust, median_amp, sort_idx, waveform)
